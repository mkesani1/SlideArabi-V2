"""
SlideShift v2 — Closed-Loop Visual Quality Assurance (VQA)

Phase 6 in the pipeline. Renders original and converted slides to images,
uses a vision model (Gemini Flash) to perform a "human-like" visual
comparison, applies deterministic remediation for actionable issues, and
logs every finding to a structured JSON Lines file for root-cause analysis.

Architecture
────────────
Phase 6 Flow:
  6a. Render original + converted slides → JPEG images
  6b. Build side-by-side composites → send to vision model → VQAReport
  6c. For each FAIL slide:
        • Map issues to RemediationActions
        • Apply surgical PPTX fixes (font resize, alignment, chevron flip, …)
        • Re-save PPTX
  6d. Re-render fixed slides → re-check with vision model (1 retry max)
...
2.  VQAConfig        — configuration dataclass
...
6.  VisionModelClient— Gemini vision API via curl (sandbox-safe)
7.  VQARemediator    — deterministic PPTX fixes per VQA issue category
8.  IssueLogger      — JSON Lines structured logging for root-cause analysis
9.  VisualQualityAssurance — main closed-loop orchestrator
10. run_vqa()        — convenience function for pipeline integration
...
class VQARating(str, Enum):
    ...
    MINOR = "MINOR"    # Small issues that don't affect usability
...
VQA_SYSTEM_PROMPT = """\
You are a visual quality assurance system for SlideShift, a tool that converts
...
class VisualQualityAssurance:
    ...
    def run(self) -> VQAReport:
        ...
        report = VQAReport(model_used=self.config.vision_model)
        ...
        try:
            ...
            # ── 6b: Select slides, build composites, run vision model ──────
            ...
            # First vision model pass
            logger.info(
                f"VQA 6b: Evaluating {len(composites)} composites with "
                f"{self.config.vision_model}..."
            )
            first_pass_results = self._evaluate_in_batches(composites)
            ...
            if fail_slides:
                ...
                # Build a partial report with just the fail slides for remediator
                ...
                # ── 6d: Re-render and re-check fixed slides ────────────────
                logger.info(
                ...
                try:
                    ...
                    retry_conv_images = conv_images  # Fall back to originals
                ...
                logger.info(
                    f"VQA 6d: Re-evaluating {len(retry_composites)} fixed slides..."
                )
                retry_results = self._evaluate_in_batches(retry_composites)

                # Merge retry results into the main results dict
                for retry_result in retry_results:
                    sn = retry_result.slide_number
                    original_result = results_by_slide.get(sn)
                    results_by_slide[sn] = retry_result

                    # Count as successful if rating improved
                    if (
                        original_result
                        ...
                        # Remediation was insufficient — note it in the log
                        logger.info(
            ...
            if self.config.issue_log_path:
                try:
                    ...
                    # Collect all issues from first pass
                    ...
                    logger.info(f"VQA 6e: Logged {total_logged} issues to {self.config.issue_log_path}")
                except Exception as e:
            ...
            # Use post-remediation results where available
            final_results = []
    ...
    def _evaluate_in_batches(
        ...
        """Send composites to the vision model in batches."""
...
def run_vqa(
    ...
    Args:
        ...
        vision_model:       Gemini model name for visual evaluation.
