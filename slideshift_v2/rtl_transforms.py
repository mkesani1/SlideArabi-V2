...
class SlideContentTransformer:
        ...
        for shape in all_shapes:
            try:
                ...
                if hasattr(shape, 'shapes') and not getattr(shape, 'is_placeholder', False):
                    # Mirror the group's position as a unit
    ...
    def _ensure_no_content_flip(self, shape) -> None:
        """
        Fix 4: Remove any flipH/flipV attributes from a shape's xfrm.
        Images (maps, photos) must never be content-flipped — only repositioned.
        """
        try:
            sp_el = shape._element
            for xfrm in sp_el.iter(f'{{{A_NS}}}xfrm'):
    ...
    def _transform_chart_rtl(self, shape) -> int:
        ...
        Operations:
        1. Reverse the category axis direction (crosses at 'max' side → reads RTL).
        2. Mirror legend position (right → left, left → right).
        3. Set value axis to right side if it was on the left.
        4. Set RTL on chart title text frame if present.

        All operations target the chart XML via lxml; python-pptx's chart
        API does not expose these properties directly.
        ...
        try:
            ...
            # Step 1: Reverse category axis direction
            # Set <c:crossBetween val="between"/> and <c:crosses val="max"/>
            # on the category axis so it reads right-to-left
            for cat_ax in chart_elem.iter(f'{{{c_ns}}}catAx'):
                crosses = cat_ax.find(f'{{{c_ns}}}crosses')
                if crosses is None:
                    crosses = etree.SubElement(cat_ax, f'{{{c_ns}}}crosses')
                crosses.set('val', 'max')

                # Set axis orientation to reverse for RTL reading
            ...
            # Step 3: Move value axis to the right if it's on the left
            for val_ax in chart_elem.iter(f'{{{c_ns}}}valAx'):
                ax_pos = val_ax.find(f'{{{c_ns}}}axPos')
                if ax_pos is not None:
                    pos = ax_pos.get('val', 'l')
                    mirror_map = {'l': 'r', 'r': 'l', 't': 'b', 'b': 't'}
                    new_pos = mirror_map.get(pos, pos)
                    ax_pos.set('val', new_pos)
                    if new_pos != pos:
                        changes += 1

            # Fix 7: Reverse value axis direction for bar charts
            # This makes horizontal bars grow right-to-left
            for val_ax in chart_elem.iter(f'{{{c_ns}}}valAx'):
                ...
                if orientation is None:
                    orientation = etree.SubElement(scaling, f'{{{c_ns}}}orientation')
                current_val = orientation.get('val', 'minMax')
                new_val = 'maxMin' if current_val == 'minMax' else 'minMax'
                orientation.set('val', new_val)
                changes += 1

            # Round 3 Step 4: Translate chart category labels and series names
            if self.translations:
                changes += self._translate_chart_labels(chart_elem, c_ns)

            # Step 5: Set RTL on chart title text
            try:
    ...
    def _fix_wrap_none_for_arabic(self, shape) -> int:
        ...
        try:
            ...
            # Expand text box width to fill available panel space.
            # The text box right edge (for RTL) should align to either:
            # - The slide half-width (for split layouts)
            # - The slide width minus a margin (for full-width layouts)
            half_width = self._slide_width // 2
            right_edge = x + cx

            # Determine which panel the shape is in
            shape_center_x = x + cx // 2
            if shape_center_x < half_width:
                # Shape is in left half — expand right edge toward half_width
                target_right = half_width - 200000  # 0.22" margin from center
            else:
                # Shape is in right half — expand right edge toward slide edge
                target_right = self._slide_width - 200000  # 0.22" margin from edge

            if target_right > right_edge:
                new_cx = target_right - x
                if new_cx > cx:
                    ext.set('cx', str(new_cx))
                    changes += 1
                    logger.debug('Fix 11: Expanded wrap=none textbox "%s" cx %d -> %d',
                                 getattr(shape, 'name', '?'), cx, new_cx)

            # Also ensure the left edge is not too far from panel boundary
            # For RTL text (right-aligned), the RIGHT edge is what matters
            # Shift the text box so its right edge aligns to the panel boundary
            new_cx_val = int(ext.get('cx', cx))
            new_right = x + new_cx_val
            if shape_center_x < half_width:
                desired_right = half_width - 200000
            else:
                desired_right = self._slide_width - 200000

            if abs(new_right - desired_right) > 500000:  # more than 0.55" off
                new_x = desired_right - new_cx_val
                if new_x >= 0:
                    off.set('x', str(new_x))
                    changes += 1
    ...
    def _fix_cover_title_anchor(self, shapes: list, slide_number: int) -> int:
        ...
        try:
            ...
            for shape in shapes:
                sp_el = shape._element
                tag = sp_el.tag
                if not (tag.endswith('}pic') or tag.endswith('}sp')):
                    continue
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Fix 15: Mirror split-panel layouts
    # ─────────────────────────────────────────────────────────────────────
