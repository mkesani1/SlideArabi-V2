"""
test_rtl_transforms.py — Unit tests for SlideShift v2 RTL transforms and typography.

Run with: python -m pytest slideshift_v2/tests/test_rtl_transforms.py -v
...
# Ensure the workspace root is on sys.path so 'slideshift_v2' is importable
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))

from slideshift_v2.utils import (
    emu_to_inches,
    emu_to_pt,
    pt_to_emu,
    inches_to_emu,
...
from slideshift_v2.rtl_transforms import (
    TransformReport,
    MasterLayoutTransformer,
    SlideContentTransformer,
)
from slideshift_v2.typography import (
...
class TestLogoDetection(unittest.TestCase):
    ...
    def _make_pic_element(self, width_fraction=0.10, has_text=False,
        ...
        if has_embed:
            ...
            # Use the proper R_NS
            from slideshift_v2.utils import R_NS
...
class TestParagraphAlignmentComputation(unittest.TestCase):
    ...
    def test_arabic_text_aligns_right(self):
        transformer = self._make_transformer()
        algn = transformer._compute_paragraph_alignment('مرحبا بالعالم العربي', None)
...
class TestBidiFormattingRules(unittest.TestCase):
    ...
    def test_ctrTitle_gets_rtl_1_and_ctr(self):
        ...
        with patch('slideshift_v2.typography.get_placeholder_info',
