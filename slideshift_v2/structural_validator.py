...
class IssueType:
    RTL_MISSING = 'rtl_missing'
    ALIGNMENT_WRONG = 'alignment_wrong'
    SHAPE_OUT_OF_BOUNDS = 'shape_out_of_bounds'
    SHAPE_OVERLAP = 'shape_overlap'
    FONT_TOO_SMALL = 'font_too_small'
    TABLE_NOT_RTL = 'table_not_rtl'
    CHART_NOT_RTL = 'chart_not_rtl'
    PLACEHOLDER_MISMATCH = 'placeholder_mismatch'
    MASTER_NO_RTL = 'master_no_rtl'
...
class StructuralValidator:
    ...
    def __init__(self, presentation, resolved_presentation=None):
        ...
        self.MIN_BODY_PT = 10
        self.MIN_ABS_PT = 8
        self.OVERLAP_TOLERANCE = 0.10
        self.BOUNDS_TOLERANCE = 100000
        ...
        for p_idx, paragraph in enumerate(shape.text_frame.paragraphs):
            ...
            if mostly_arabic and rtl_attr != '1':
                issues.append(ValidationIssue(
                    severity='error', slide_number=slide_number,
                    ...
                    expected_value='1', actual_value=rtl_attr
                ))
            elif not mostly_arabic and rtl_attr != '1':
                issues.append(ValidationIssue(
                    severity='warning', slide_number=slide_number,
                    shape_id=shape.shape_id, shape_name=shape.name,
                    issue_type=IssueType.RTL_MISSING,
        ...
        for p_idx, paragraph in enumerate(shape.text_frame.paragraphs):
            ...
            if alignment == PP_ALIGN.LEFT:
                issues.append(ValidationIssue(
                    ...
                    expected_value='RIGHT or JUSTIFY', actual_value='LEFT'
                ))
                ...
                issues.append(ValidationIssue(
                    ...
                    expected_value='RIGHT or JUSTIFY', actual_value='CENTER'
                ))
        return issues
    ...
    def _check_shape_bounds(self, shape, slide_number: int) -> List[ValidationIssue]:
        ...
        if (shape.left < left_bound or shape.top < top_bound or
            ...
            issues.append(ValidationIssue(
                severity=severity, slide_number=slide_number,
                shape_id=shape.shape_id, shape_name=shape.name,
            ...
            for j in range(i + 1, len(shapes_data)):
                ...
                if intersection > 0:
                    ...
                    if self.OVERLAP_TOLERANCE < overlap_pct < 0.90:
                        ...
                        issues.append(ValidationIssue(
                            severity=severity, slide_number=slide_number,
                            shape_id=s1.shape_id, shape_name=s1.name,
        ...
        if tblPr is not None:
            ...
            if rtl != '1' and rtl != 'true':
                issues.append(ValidationIssue(
                    ...
                    expected_value='1', actual_value=rtl
                ))
        else:
            issues.append(ValidationIssue(
                severity='error', slide_number=slide_number,
            ...
            if chart.has_category_axis:
                ...
                if orientations and orientations[0] != 'maxMin':
                    issues.append(ValidationIssue(
                        ...
                        expected_value='maxMin', actual_value=orientations[0]
                    ))
        except Exception:
            pass
        return issues
                ...
                if run.font.size is not None:
                    ...
                    if size_pt < self.MIN_ABS_PT:
                        issues.append(ValidationIssue(
                            ...
                            message=f"Font size {size_pt}pt is below absolute minimum ({self.MIN_ABS_PT}pt)",
                            expected_value=f">={self.MIN_ABS_PT}", actual_value=size_pt
                        ))
                    elif size_pt < min_pt:
                        issues.append(ValidationIssue(
                            severity='warning', slide_number=slide_number,
                            shape_id=shape.shape_id, shape_name=shape.name,
                            issue_type=IssueType.FONT_TOO_SMALL,
                            message=f"Font size {size_pt}pt is smaller than recommended minimum ({min_pt}pt)",
                            expected_value=f">={min_pt}", actual_value=size_pt
                        ))
        return issues
        ...
        for i, master in enumerate(self.prs.slide_masters):
            ...
            for style_name in ['titleStyle', 'bodyStyle', 'otherStyle']:
                ...
                if lvl1pPr:
                    ...
                    if rtl != '1':
                        issues.append(ValidationIssue(
                            severity='warning', slide_number=0,
                            shape_id=None, shape_name=f"Master {i+1}",
