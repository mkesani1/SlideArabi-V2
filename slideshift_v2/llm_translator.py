#!/usr/bin/env python3
"""
SlideShift v2 — Dual-LLM Translation Backend
═════════════════════════════════════════════
...
class GPTClient:
    ...
    def translate_batch(self, numbered_texts: Dict[str, str]) -> Dict[str, str]:
        ...
        status, response_text = _curl_post(
            "https://api.openai.com/v1/chat/completions",
            headers, body, self.timeout
        )

        if status != 200:
            logger.error("GPT API error %d: %s", status, response_text[:500])
            raise RuntimeError(f"GPT API returned status {status}")
...
class ClaudeClient:
    ...
    def qa_batch(self, pairs: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        if status != 200:
            logger.error("Claude API error %d: %s", status, response_text[:500])
            raise RuntimeError(f"Claude API returned status {status}")
...
class DualLLMTranslator:
    ...
    def _gpt_translate_batched(
        ...
        for batch_start in range(0, len(items), batch_size):
            ...
            logger.info("GPT batch %d/%d (%d strings)",
                       batch_num, total_batches, len(batch))

            # Re-number within batch for cleaner prompts
            renumbered = {}
            idx_map = {}  # batch_idx → original_idx
            ...
            for attempt in range(self.config.max_retries + 1):
                try:
                    ...
                    for batch_idx, arabic in batch_result.items():
                        orig_idx = idx_map.get(str(batch_idx), batch_idx)
                    ...
                    if attempt < self.config.max_retries:
                        ...
                        # Fall back: mark as untranslated
                        error_msg = f"GPT batch {batch_num} failed after {self.config.max_retries + 1} attempts: {e}"
                        self._report.errors.append(error_msg)
                        logger.error(error_msg)
                        for batch_idx, orig_idx in idx_map.items():
    ...
    def _claude_qa_pass(
        ...
        for batch_start in range(0, len(pairs), qa_batch_size):
            ...
            try:
                ...
                error_msg = f"Claude QA batch {batch_num} failed: {e}"
                self._report.errors.append(error_msg)
                logger.warning(error_msg)

            # Small delay between QA batches
            if batch_start + qa_batch_size < len(pairs):
                time.sleep(0.5)
