...
└─────────────────┬───────────────────┘
       ...
       │  │ GPT-5.2  EN→AR translation       │ │
       │  │ Claude Sonnet 4.6 QA pass       │ │
...
process_single_deck.py  (standalone runner — bypasses pipeline.py)
 ...
 └── llm_translator.py      (Phase 1b — dual-LLM, activated by --llm-translate)
      ...
      ├── GPT-5.2             (primary EN→AR translation)
...
| `LLMTranslator` | Orchestrates the full three-layer pipeline: TokenProtector → GPT-5.2 translation → Claude QA pass → domain glossary overrides. |
| `ClaudeQAPass` | Post-processing layer. Submits EN→AR pairs to Claude Sonnet 4.6 for terminology and semantic review. |
...
Replaces tokens that must not be translated with opaque `⟦PROTXXXX⟧` placeholders before the text reaches GPT-5.2. Restores originals after translation.
...
Reviews all EN→AR translation pairs in batches of 60 using Claude Sonnet 4.6. Identifies and corrects six issue categories.
...
| GPT-5.2 only | 8.8 / 10 |
| GPT-5.2 + Claude QA | **9.5 / 10** |

All Category A failures eliminated (HW→"hazardous waste", brand name corruption, semantic inversions).

**Model selection rationale:**

- **GPT-5.2** was chosen as primary translator based on benchmarks: BLEU 54.2, COMET 0.847, Human Accuracy 4.3/5 — strongest overall EN→AR quality among frontier models tested.
- **Claude Sonnet 4.6** was chosen as QA reviewer to provide independent verification. A model reviewing its own output catches fewer errors than a different model reviewing the same output.
...
- `OPENAI_API_KEY` — required when `--llm-translate` is active; passed to GPT-5.2 calls
- `ANTHROPIC_API_KEY` — optional; if set, enables the Claude Sonnet 4.6 QA pass. If absent, `LLMTranslator` skips Layer 2 and runs GPT-only mode.
...
# Dual-LLM mode (Phase 1b — GPT-5.2 + Claude QA)
python process_single_deck.py <input.pptx> <output.pptx> --llm-translate

# Dual-LLM mode, GPT-5.2 only (skip Claude QA pass)
...
# Process all 16 decks with GPT-5.2 only (no Claude QA pass)
...
Phase 1b is an optional, drop-in replacement for the Google Translate (GTX) translation step. When activated via `--llm-translate`, it replaces the curl-based GTX calls with a three-layer pipeline using GPT-5.2 as the primary translator and Claude Sonnet 4.6 as an independent QA reviewer. The output is a translation map in exactly the same JSON format as existing GTX caches, so no other pipeline stage requires modification.
...
**Problem it solves:** GPT-5.2 — despite explicit instructions — will sometimes transliterate abbreviations (`HW` → `هو`), translate financial codes (`EBITDA` → Arabic expansion), or mangle numeric identifiers (`Q1-24` → `الربع الأول 24`). These are Category A failures that produce unintelligible Arabic in executive presentations.
...
**Problem it solves:** Even with TokenProtector active, GPT-5.2 produces errors in approximately 25% of strings in complex business decks — wrong terminology, gender agreement failures, inconsistent translation of the same term, overly literal phrasing that changes business meaning.

**Solution:** After GPT translation, all EN→AR pairs are submitted to Claude Sonnet 4.6 for independent review. Claude is a different model with different training data and different translation tendencies. Errors that GPT won't self-correct are reliably caught by Claude's independent review pass.
...
**GPT-5.2 system prompt (2,013 chars) — key rules:**
...
| GPT-5.2 batches per deck | ~2 (40 strings/batch) |
| Claude batches per deck | ~1 (60 pairs/batch) |
| GPT-5.2 cost per deck | ~$0.02 |
| Claude Sonnet 4.6 cost per deck | ~$0.07 |
...
| GPT-5.2 only | 8.8 | 0 (TokenProtector eliminates Category A) |
| GPT-5.2 + Claude QA | **9.5** | 0 |

Category A failures (HW → hazardous waste, brand name corruption, semantic inversions) are eliminated at the TokenProtector and GPT-5.2 layer. The Claude QA pass drives the score from 8.8 to 9.5 by catching the subtler Category B failures (register, consistency, partial semantic errors).
...
|----------|----------|--------|
| `OPENAI_API_KEY` | Yes (when `--llm-translate`) | Authenticates GPT-5.2 API calls |
